...
// ============================================================================
//
// Key:     B-sdk-error-catalog
// Purpose: Centralized error codes for CPI-SI SDK
//

pub static PRAGMA: &[(&str, &str)] = &[
    ("I1.key", "B-sdk-error-catalog"),
    ("I1.format", "rust"),
    ("I1.from", "src/error/catalog.rs"),
    ("I1.at", "a-01.00"),
    ("I2.type", "code"),
...
pub static METADATA: &[(&str, &str)] = &[
    ...
    ("C5.purpose", "Centralized error codes for CPI-SI SDK"),
    ("C5.philosophy", "[philosophy]"),
    ("C6.current", "a-01.00 — Centralized error codes for CPI-SI SDK"),
    ("C6.planned", "[planned]"),
...
/// Structured error codes for the CPI-SI SDK.
/// Follows the CWS-T00-xxx format for tool infrastructure errors.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ErrorCode {
    /// CWS-T01-001: Substrate map file not found or unreadable.
    MapNotFound,
    /// CWS-T01-002: Substrate map file contains invalid TOML/YAML/JSON.
    MapParseError,
    /// CWS-T01-003: Incoming JSON payload is invalid or malformed.
    PayloadParseError,
    /// CWS-T01-004: Mapping rule failed to resolve against payload.
    RuleResolutionFailed,
    /// CWS-T01-005: Unknown or unregistered substrate requested.
    UnknownSubstrate,
    /// CWS-T01-006: Requested event not defined in substrate map.
    EventNotDefined,
    /// CWS-T01-007: General mapping or logic error during translation.
    MappingError,
    /// CWS-T01-008: I/O error during file operations.
    IoError,
}
...
impl ErrorCode {
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            ErrorCode::MapNotFound => "CWS-T01-001",
            ErrorCode::MapParseError => "CWS-T01-002",
            ErrorCode::PayloadParseError => "CWS-T01-003",
            ErrorCode::RuleResolutionFailed => "CWS-T01-004",
            ErrorCode::UnknownSubstrate => "CWS-T01-005",
            ErrorCode::EventNotDefined => "CWS-T01-006",
            ErrorCode::MappingError => "CWS-T01-007",
            ErrorCode::IoError => "CWS-T01-008",
        }
    }

    /// Returns the default message template for the error code.
    pub fn message_template(&self) -> &'static str {
        match self {
            ErrorCode::MapNotFound => "Substrate map not found: {{path}}",
            ErrorCode::MapParseError => "Failed to parse map: {{details}}",
            ErrorCode::PayloadParseError => "Invalid JSON payload: {{details}}",
            ErrorCode::RuleResolutionFailed => "Rule '{{rule}}' failed to resolve against payload.",
            ErrorCode::UnknownSubstrate => "Unknown substrate: '{{substrate}}'. Ensure map is loaded.",
            ErrorCode::EventNotDefined => "Event '{{event}}' not defined for substrate '{{substrate}}'.",
            ErrorCode::MappingError => "General mapping error: {{details}}",
            ErrorCode::IoError => "I/O error: {{details}}",
        }
    }
}
...
mod identity_tests {
    ...
    fn test_identity_accessors() {
        assert_eq!(pragma_get("I1.key"), Some("B-sdk-error-catalog"));
...
// X5: Note
// ──────────────────────────────────────────────────────────────────────────
// Note: Centralized error codes for CPI-SI SDK
