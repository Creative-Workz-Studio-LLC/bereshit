...
export async function runCliCommand(command: string, args: string[] = []): Promise<void> {
  ...
  if (adapter.getMode() === 'web' && 'isConnected' in adapter && !(adapter as any).isConnected()) {
...
function expandOutputPanel(): void {
...
function appendSuccessMessage(message: string): void {
  const body = document.querySelector('.output-body');
  if (!body) return;
