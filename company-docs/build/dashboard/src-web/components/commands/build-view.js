...
export function createBuildView() {
    ...
    el.appendChild(createCommandCard({
        ...
        onRun: () => {
            ...
            if (fmt !== 'pdf')
                vars['FORMAT'] = fmt;
            if (parallelToggle.isChecked())
                vars['PARALLEL'] = '1';
            if (cleanToggle.isChecked())
                vars['CLEAN'] = '1';
    ...
    el.appendChild(createCommandCard({
        ...
        onRun: () => {
            ...
            if (!file) {
                alert('Select a section file first.');
                return;
            }
            const vars = { FILE: file };
            if (sectionForce.isChecked())
                vars['FORCE'] = '1';
    ...
    el.appendChild(createCommandCard({
        ...
        onRun: () => {
            ...
            if (!id) {
                alert('Enter an article ID.');
                return;
            }
            const args = [id];
            if (articleForce.isChecked())
                args.push('--force');
    ...
    el.appendChild(createCommandCard({
        ...
        onRun: () => {
            ...
            if (!id) {
                alert('Enter a part ID.');
                return;
            }
            const args = [id];
            if (partForce.isChecked())
                args.push('--force');
    ...
    el.appendChild(createCommandCard({
        ...
        onRun: () => {
            ...
            if (!file || !range) {
                alert('File and page range required.');
                return;
            }
