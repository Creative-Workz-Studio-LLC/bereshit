...
export const formatsEditor: EditorDefinition = {
  sectionKey: 'formats',
  title: 'Formats',
  description: 'Output format configuration (PDF, EPUB, HTML).',

  buildForm(data: unknown) {
    const formats = (data ?? {}) as Record<string, FormatConfig>;
    const el = document.createElement('div');
    const getters: Array<{ key: string; getData: () => FormatConfig }> = [];
    ...
    return {
      ...
      getData: () => {
        ...
        for (const g of getters) result[g.key] = g.getData();
        return result;
      },
    };
  },
