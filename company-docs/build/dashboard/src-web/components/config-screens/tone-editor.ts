...
function buildVoiceTab(d: AnyData): { el: HTMLElement; getData: () => unknown } {
  const el = document.createElement('div');
  const voice = d.cws_voice ?? [];

  const rows: Array<{
    name: ReturnType<typeof configInput>;
    description: ReturnType<typeof configInput>;
    row: HTMLElement;
  }> = [];
  ...
  function addRow(attr: AnyData): void {
  ...
  for (const attr of voice) addRow(attr);
...
function buildSpectrumTab(d: AnyData): { el: HTMLElement; getData: () => unknown } {
  const el = document.createElement('div');
  const spectrum = d.tone_spectrum ?? [];

  const rows: Array<{
    level: ReturnType<typeof configInput>;
    description: ReturnType<typeof configInput>;
    row: HTMLElement;
  }> = [];
  const container = document.createElement('div');
...
function buildCovenantTab(d: AnyData): { el: HTMLElement; getData: () => unknown } {
  const el = document.createElement('div');
  const covenant = d.covenant_language ?? {};
  const replacements = covenant.replacements ?? [];

  const rows: Array<{
    from: ReturnType<typeof configInput>;
    to: ReturnType<typeof configInput>;
    row: HTMLElement;
  }> = [];
  const container = document.createElement('div');
  ...
  function addRow(r: AnyData): void {
    const row = document.createElement('div');
  ...
  for (const r of replacements) addRow(r);
...
function buildPlainTab(d: AnyData): { el: HTMLElement; getData: () => unknown } {
  const el = document.createElement('div');
...
function buildReadabilityTab(d: AnyData): { el: HTMLElement; getData: () => unknown } {
  const el = document.createElement('div');
  const targets = d.readability_targets ?? [];

  const rows: Array<{
    register: ReturnType<typeof configInput>;
    grade_level: ReturnType<typeof configNumber>;
    row: HTMLElement;
  }> = [];
  const container = document.createElement('div');
  container.style.cssText = 'display: flex; flex-direction: column; gap: var(--space-3);';
...
function buildMiscTab(d: AnyData): { el: HTMLElement; getData: () => AnyData } {
  const el = document.createElement('div');
...
export const toneEditor: EditorDefinition = {
