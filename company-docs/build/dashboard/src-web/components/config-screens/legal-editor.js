 ...
 */
import { configInput, configStringArray, configKeyValue, configSection, configTabs, } from './editor-base.js';
// ── Subsection builders ──────────────────────────────────────────
function buildObligation(d) {
    ...
    return {
        el,
        getData: () => {
            const result = { register_map: regMap.getValue() };
            for (const g of groups)
                result[g.key] = { words: g.getter.getValue() };
            return result;
        },
    };
}
function buildDefinedTerms(d) {
    const el = document.createElement('div');
...
function buildSimpleStrings(d, key, label) {
    const el = document.createElement('div');
    const section = d[key] ?? {};
