...
//omni:key B-L2-util-term-debugging
...
// ────────────────────────────────────────────────────────────────────────────
// Config — database-backed health emoji with hardcoded fallback
// ────────────────────────────────────────────────────────────────────────────

// HealthEmojiEntry maps a minimum health percentage to an emoji and label.
type HealthEmojiEntry struct {
	Min   int
	Emoji string
	Label string
}

// Config holds debugging display configuration.
type Config struct {
	HealthEmoji []HealthEmojiEntry
}

// DefaultConfig returns hardcoded health emoji matching display.toml.
...
// ────────────────────────────────────────────────────────────────────────────
// B.6 Registry Integration
// ────────────────────────────────────────────────────────────────────────────

// ConfigFromRegistry builds a Config from the L2 platform database
// via the cross-layer registry. Health levels provide both the threshold
// boundaries and the display emoji.
func ConfigFromRegistry(r *registry.Registry) Config {
	c := DefaultConfig()

	if levels, err := r.Platform.AllHealthLevels(); err == nil && len(levels) > 0 {
		entries := make([]HealthEmojiEntry, 0, len(levels))
		for _, l := range levels {
			entries = append(entries, HealthEmojiEntry{
				Min:   l.MinStored,
				Emoji: l.Emoji,
				Label: l.Name,
			})
		}
		// Sort descending by Min — highest threshold first
		for i := 0; i < len(entries)-1; i++ {
			for j := i + 1; j < len(entries); j++ {
				if entries[j].Min > entries[i].Min {
					entries[i], entries[j] = entries[j], entries[i]
				}
			}
		}
		c.HealthEmoji = entries
	}
...
func healthInfo(h int) (emoji string, text string) {
	for _, hi := range cfg.HealthEmoji {
		if h >= hi.Min {
			return hi.Emoji, hi.Label
		}
	}
	return "☠️", "Failed"
}
