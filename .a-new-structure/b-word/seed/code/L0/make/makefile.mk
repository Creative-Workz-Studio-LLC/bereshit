...
#
# --- P1: Core ---
# key:       B-seed-code-make-template
# from:      None — canonical source template
# at:        b-01.00
#
# --- P2: Family ---
# type:      template
...
# provides:  MAKEFILE_TEMPLATE
#
# --- P5: Summary ---
# title:     Project Makefile Template
# brief:     4-block build orchestration template for any project
#
...
#         E10  Missing required argument
#         E11  Invalid argument value
#         E12  Conflicting arguments
#
#       E2x  Dependency errors — files, tools, paths
#         E20  Source file not found
#         E21  Tool not found
#         E22  Directory not found
#         E23  No matching files (glob returned empty)
#
#       E3x  Build errors — compilation, rendering
#         E30  Build failed (non-zero exit)
#         E31  Asset generation failed
#         E33  TypeScript/Node build failed
#         E34  Partial build — some files failed in batch
#
#       E4x  Validation errors — structural, editorial
#         E40  Validation failure — structure checks found errors
#         E41  Lint failure (style violations found)
#
# What This Needs:
...
#   toml:         None
#   json:         None
#   editorconfig: editorconfig.editorconfig — co-located formatting template
#   makefile:     None — this is the canonical source template
...
#   .PHONY: setup config                     # maintenance targets
#   .PHONY: status info                      # information targets
...
#
#   Configuration lives in a config source (YAML, TOML, etc.).
#   Run `make config` to regenerate config.mk from the source.
#
#   include config.mk                        # shared configuration (required)
#   -include config.mk                       # shared configuration (silent if missing)
#   -include local.mk                        # local overrides (not committed)
#   include $(BUILD_DIR)/deps.mk             # generated dependencies
#
#   To change behavior:  Edit config source, then run `make config`.
#   Bootstrap (no config.mk):  ?= defaults in §1 ensure the Makefile always works.
...
#
#   Use ?= for values that config.mk may override.
#   Use := for values that should never be overridden.
#
#   BINARY_NAME ?= [name]            # output binary name (overridable)
#   VERSION     ?= 1.0.0             # project version (overridable)
#   BUILD_DIR   := bin               # build output (fixed)
#   PREFIX      ?= /usr/local        # installation prefix (overridable)
...
#
#   Dependency check define — reusable tool verification.
#   Uses exit code E21 (tool not found) from the exit code system.
#
#   Usage:
#     $(call check_dep,go)
#     $(call check_dep,gcc)
...
# # B.2c Cache Engine [CACHE]
#
#   Purpose:  Skip rebuilds when sources haven't changed
#   Pattern:  Timestamp-based — compare output mtime against newest source
#   Override: FORCE=1 on any target to force rebuild
#
#   Usage in recipes:
#     CACHE_OUT="path/to/output"; \
...
if [ "$(FORCE)" = "1" ]; then \
    ...
    NEWEST=$$(find $$CACHE_SRCS -type f 2>/dev/null | xargs ls -t 2>/dev/null | head -1); \
...
# # B.3e Config Generation [CONFIG]
#
#   ## config: Regenerate config.mk from config source
#   config:
#   	@echo "Regenerating config.mk..."
#   	@# [Config generation command — project specific]
#   	@# node scripts/generate-config.js > config.mk
#   	@# python scripts/yaml2mk.py build.config.yaml > config.mk
#   	@echo "Config regenerated"

# [Reserved: Uncomment when config source exists]
# ## config: Regenerate config.mk from config source
# config:
# 	@echo "Regenerating config.mk..."
# 	@# [generation command]
# 	@echo "Config regenerated: config.mk"

# # B.3f Maintenance [MAINTENANCE]
#
...
## setup: Install and verify required dependencies
setup:
	@echo "Checking dependencies..."
	@# [Uncomment tools your project requires]
	@# $(call check_dep,make)
	@# $(call check_dep,go)
	@# $(call check_dep,gcc)
...
## status: Show build state (binary exists, size, timestamp)
status:
	...
	@if [ -f "$(BUILD_DIR)/$(BINARY_NAME)" ]; then \
	    ...
	    MOD=$$(stat -c '%y' "$(BUILD_DIR)/$(BINARY_NAME)" 2>/dev/null | cut -d. -f1); \
	    echo "  Binary: $(BUILD_DIR)/$(BINARY_NAME) ($$SIZE, $$MOD)"; \
	else \
	    echo "  Binary: not built"; \
	fi

## deps: Check all dependencies (required + optional)
deps:
	@echo "Required:"
	@# $(call check_dep,make)
	@# $(call check_dep,[tool])
	@echo ""
	@echo "Optional:"
	@# command -v entr >/dev/null 2>&1 && echo "  entr: found" || echo "  entr: not found (watch mode)"
	@echo ""
