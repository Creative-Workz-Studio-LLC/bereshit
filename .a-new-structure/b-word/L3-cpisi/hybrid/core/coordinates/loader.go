...
//omni:key B-L3-coordinates-loader
...
var (
	globalBibleData *BibleData
	bibleDataOnce   sync.Once
	bibleDataErr    error
)

// CachedBibleData returns cached Bible data (lazy loaded from default Bereshit dir).
// Thread-safe via sync.Once. Immutable data — safe to cache permanently.
func CachedBibleData() (*BibleData, error) {
	bibleDataOnce.Do(func() {
		globalBibleData, bibleDataErr = LoadBibleData(DefaultBereshitDir())
	})
	return globalBibleData, bibleDataErr
}

// MustLoadBibleData loads Bible data or panics.
// Uses L0 generic Must wrapper.
func MustLoadBibleData(bereshitDir string) *BibleData {
	return util.Must(LoadBibleData(bereshitDir))
}
...
func LoadBibleData(bereshitDir string) (*BibleData, error) {
	...
	if _, err := os.Stat(bibleDir); err != nil {
		return nil, util.NewLoadError(bibleDir, "find", err)
	}
	...
	if err := loadEncoding(bibleDir, data); err != nil {
		return nil, err // *util.LoadError with file + op
	}
...
func loadEncoding(bibleDir string, data *BibleData) error {
	...
	if _, err := toml.DecodeFile(path, &raw); err != nil {
		return util.NewLoadError(EncodingFile, "parse", err)
	}
...
func loadDecoding(bibleDir string, data *BibleData) error {
	...
	if _, err := toml.DecodeFile(path, &raw); err != nil {
		return util.NewLoadError(DecodingFile, "parse", err)
	}
...
func loadAddressing(bibleDir string, data *BibleData) error {
	...
	if _, err := toml.DecodeFile(path, &raw); err != nil {
		return util.NewLoadError(AddressingFile, "parse", err)
	}
...
func LoadCubeIndex(bereshitDir string, set TranslationSet) (*CubeIndex, error) {
	...
	if err != nil {
		return nil, util.NewLoadError(path, "read", err)
	}

	var idx CubeIndex
	if err := json.Unmarshal(data, &idx); err != nil {
		return nil, util.NewLoadError(path, "parse", err)
	}
