//omni:code --go -test
//omni:key B-L3-bereshit-loader_test
//omni:version a-01.00
package bereshit_test
...
// ============================================================================
//
// Bereshit Assurance Tests — Wrappers for go test compatibility
//
...
// Default Bereshit directory
var testDir = bereshit.DefaultBereshitDir()
...
func TestAssureTransliteration(t *testing.T) {
	result := bereshit.AssureTransliteration(testDir)
	logDAR(t, result)
	requirePass(t, result)
}

func TestAssureHebrew(t *testing.T) {
	trans, err := bereshit.LoadTransliteration(testDir)
	if err != nil {
		t.Fatalf("Failed to load transliteration: %v", err)
	}
	result := bereshit.AssureHebrew(trans)
	logDAR(t, result)
	requirePass(t, result)
}

func TestAssureGreek(t *testing.T) {
	trans, err := bereshit.LoadTransliteration(testDir)
	if err != nil {
		t.Fatalf("Failed to load transliteration: %v", err)
	}
	result := bereshit.AssureGreek(trans)
	logDAR(t, result)
	requirePass(t, result)
}

func TestAssureCubeGeometry(t *testing.T) {
	trans, err := bereshit.LoadTransliteration(testDir)
	if err != nil {
		t.Fatalf("Failed to load transliteration: %v", err)
	}
	result := bereshit.AssureCubeGeometry(trans)
	logDAR(t, result)
	requirePass(t, result)
}

func TestAssureStates(t *testing.T) {
	trans, err := bereshit.LoadTransliteration(testDir)
	if err != nil {
		t.Fatalf("Failed to load transliteration: %v", err)
	}
	result := bereshit.AssureStates(trans)
	logDAR(t, result)
	requirePass(t, result)

	// HALT_10 is critical - 7 Days of Creation
	if result.Context["state_count"] != "7" {
		t.Errorf("HALT_10 VIOLATED: Expected 7 states (7 Days), got %s", result.Context["state_count"])
	}
}
...
func TestRunAllAssurances(t *testing.T) {
	suite := bereshit.RunAllAssurances(testDir)

	// Header
	t.Logf("═══════════════════════════════════════════════════════════")
	t.Logf(" BERESHIT ASSURANCE SUITE")
	t.Logf("═══════════════════════════════════════════════════════════")
