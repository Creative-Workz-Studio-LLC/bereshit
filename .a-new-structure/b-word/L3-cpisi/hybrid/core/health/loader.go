...
// MustConfig returns the global health configuration or panics.
// Uses L0 generic Must wrapper.
func MustConfig() *HealthConfig {
	return util.Must(Config())
}
...
func LoadHealthConfig(schemaDir string) (*HealthConfig, error) {
	...
	if err := loadLogConfig(logPath, config); err != nil {
		// Non-fatal - use defaults, but emit tripwire warning
		util.TripwireWarning("health", logPath, "using hardcoded defaults for ternary/actions")
...
// Normalized to toml.DecodeFile (consistent with all other loaders).
