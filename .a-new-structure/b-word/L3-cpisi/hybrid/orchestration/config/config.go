...
// THROUGHLINE: SchemaError and JSONCError are now type aliases for util.LoadError.
// ValidationError is a type alias for util.ValidationError.
// The error format changes slightly (unified "load" prefix) — this is intentional,
// as schema/JSONC are both file-loading operations.

// SchemaError wraps schema file loading failures.
// Delegated to util.LoadError — single source of truth.
type SchemaError = util.LoadError

// JSONCError wraps JSONC file loading failures.
// Delegated to util.LoadError — single source of truth.
type JSONCError = util.LoadError

// ValidationError represents a single validation error.
// Delegated to util.ValidationError — single source of truth.
...
// ParseJSONC delegates to util.ParseJSONC (throughline consolidation).
...
func ParseJSONC(content string) (map[string]interface{}, error) {
	cleaned, err := util.ParseJSONC([]byte(content))
	if err != nil {
