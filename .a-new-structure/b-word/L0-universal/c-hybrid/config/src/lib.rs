...
pub use identity::{metadata, pragma};
...
"#,
    ...
    fn test_pragma_and_metadata() {
        let p = identity::pragma();
        assert_eq!(p["P1.key"], "B-L0-hybrid-config");
        assert_eq!(p["P1.format"], "rs");

        let m = identity::metadata();
        assert_eq!(m["M1.key"], "B-L0-hybrid-config");
        assert_eq!(m["M6.layer"], "L0-universal");
    }

    #[test]
    fn test_identity_returns_copies() {
        let mut p1 = identity::pragma();
        let p2 = identity::pragma();
        p1.insert("P1.key".into(), "mutated".into());
        // p2 should NOT be affected
        assert_eq!(p2["P1.key"], "B-L0-hybrid-config");
    }
