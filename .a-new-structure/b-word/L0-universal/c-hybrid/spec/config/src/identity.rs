...
//omni:key B-L0-hybrid-config-identity
...
pub static PRAGMA: &[(&str, &str)] = &[
    ...
    (
        "I3.brief",
        "Manifest-driven TOML spec loader with format dispatch and utilities",
    ),
    // I4: Architecture
    ("I4.layer", "L0"),
    ...
    (
        "I4.pattern",
        "manifest-driven loading — index.toml declares what to load",
    ),
];
...
pub static METADATA: &[(&str, &str)] = &[
    ...
    (
        "C3.principle",
        "Configuration is vision made plain — declare what exists, the system loads it",
    ),
    (
        "C3.anchor",
    ...
    (
        "C4.requires.stdlib",
        "std::collections, std::io, std::path, std::sync",
    ),
    ("C4.requires.external", "toml 0.8, serde 1, serde_json 1"),
    (
        "C4.requires.internal",
        "bereshit-l0-paths (root discovery), bereshit-l0-identity (registration)",
    ),
    (
        "C4.consumers",
        "L3 orchestration, schema validation, any crate loading TOML config",
    ),
    (
        "C4.integration",
        "use bereshit_l0_config::{load_toml_file, find_config_dir, pragma_get}",
    ),
    (
        "C4.if_missing",
        "every crate implements its own TOML loading — duplicated effort, inconsistent behavior",
    ),
    // C5: Intent
    (
        "C5.purpose",
        "Unified config loading — manifest-driven with format dispatch and utilities",
    ),
    (
        "C5.philosophy",
        "L0 stays universal — no OmniCode, no L1+ deps. Faithful Go port using Rust idioms.",
    ),
    // C6: Roadmap
    (
        "C6.current",
        "b-03.00 — 14 modules, typed errors, 87 tests, production structure",
    ),
    ("C6.planned", "YAML loading, schema validation helpers"),
    (
        "C6.limitations",
        "File I/O only, no network, no streaming, no watch/hot-reload",
    ),
    // C7: Classification
    (
        "C7.tags",
        "config, toml, jsonc, formats, loader, exists, must, identity",
    ),
...
mod tests {
    ...
    fn test_pragma_and_metadata() {
        ...
        assert_eq!(metadata_get("C1.version"), Some("b-03.00"));
        assert_eq!(metadata_get("C7.domain"), Some("config-loading"));
    }
