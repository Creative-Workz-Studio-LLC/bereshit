...
//! ## Tables
//!
//! | Table | Content | Key Query |
//! |-------|---------|-----------|
//! | `specs` | TOML spec metadata | [`DB::spec_by_key`], [`DB::all_specs`] |
//! | `type_families` | 10 primitive families | [`DB::all_families`] |
//! | `types` | 23+ primitive types | [`DB::type_by_name`], [`DB::all_types`] |
//! | `operations` | Ternary arithmetic | [`DB::all_operations`] |
//! | `books` | 66 Bible books | [`DB::all_books`], [`DB::book_by_ordinal`] |
//! | `ternary_scales` | Named scales | [`DB::all_scales`] |
//! | `dependencies` | Spec cross-refs | [`DB::dependencies_of`] |
//! | `web_variants` | WEB-only verses | [`DB::all_web_variants`] |
//!
//! ## Usage
//!
//! ```no_run
//! use bereshit_l0_foundation_db::DB;
//!
//! let db = DB::open("/path/to/b-word/data/foundation/database")?;
//! let trit = db.type_by_name("trit")?;
//! assert_eq!(trit.family, "trit");
//! # Ok::<(), bereshit_l0_foundation_db::FoundationError>(())
//! ```
//!
//! ## OmniCode Identity
//!
...
//! |--------|----------|---------|-------|
//! | [`identity::PRAGMA`] | I1-I4 | Identity — who this file is | [`pragma_get`] |
//! | [`identity::METADATA`] | C1-C7 | Context — when, where, why, how | [`metadata_get`] |
//!
...
// ============================================================================
//
// Cv — Closing Validation: tests below verify all 8 table readers + identity.
//
// Ce — Library crate. Registration via identity::register_identity().
//       Read-only database access — no writes, no migrations, no pooling.
//
...
//     - Remove bundled SQLite feature (ensures zero system dependencies)
//
//   Careful:
...
//   Every type, operation, scale, book, and dependency — discoverable by query,
//   not hardcoded. The system knows itself.
//
...
mod tests {
    ...
    fn pragma_identity() {
        ...
        assert_eq!(pragma_get("I4.layer"), Some("L0"));
        assert_eq!(pragma_get("Z99.missing"), None);
    }

    #[test]
    fn metadata_identity() {
        assert_eq!(metadata_get("C7.domain"), Some("foundation-queries"));
        assert_eq!(metadata_get("C7.paradigm"), Some("CPI-SI"));
        assert_eq!(metadata_get("C1.status"), Some("Active"));
        assert_eq!(metadata_get("Z99.missing"), None);
    }
    ...
    fn identity_completeness() {
        for prefix in ["I1.", "I2.", "I3.", "I4."] {
            assert!(
                identity::PRAGMA.iter().any(|(k, _)| k.starts_with(prefix)),
                "missing pragma section {prefix}"
            );
        }
        for prefix in ["C1.", "C2.", "C3.", "C4.", "C5.", "C6.", "C7."] {
            assert!(
                identity::METADATA.iter().any(|(k, _)| k.starts_with(prefix)),
    ...
    #[test]
    fn register_identity_with_l0() {
        register_identity();
        let pkg = bereshit_l0_identity::lookup("B-L0-foundation-db");
        assert!(pkg.is_some(), "foundation-db should be registered");
        let pkg = pkg.unwrap();
    ...
    fn error_display_not_found() {
        let err = FoundationError::NotFound("test_query".into());
        let msg = format!("{err}");
        assert!(msg.contains("test_query"));
        assert!(msg.contains("not found"));
    }

    #[test]
    fn open_nonexistent_dir() {
        let result = DB::open("/nonexistent/path/to/db");
        match result {
            Err(FoundationError::Open(path, _)) => {
                assert!(path.contains("nonexistent"), "error should contain the path");
            }
