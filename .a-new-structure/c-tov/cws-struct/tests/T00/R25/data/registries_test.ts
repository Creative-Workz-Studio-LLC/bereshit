...
//          Gate  | Verifies
//          ------|---------
//          G1    | Shape — every section has exactly 11 concept_map keys
//          G2    | Keys — the 11 keys match the canonical concept list
...
//          G10   | Counts — 42 sections, 11 concepts, 5 forms
//
...
import { SECTION_REGISTRY, SECTION_ORDER } from "@lib/data/sections.ts";
...
/** The 11 canonical concept IDs. Every section must have exactly these keys. */
...
/** Total expected concept count. */
const TOTAL_CONCEPTS = 11;
...
Deno.test("data/registries: test infrastructure ready — gates defined", () => {
...
Deno.test("data/registries: recalculateWeight — 1G 1F 9D = 0.73", () => {
  // Constants: variable_binding=granted, string_format=defer, rest=denied
  const map: Record<string, TernaryValue> = {};
  for (const c of CANONICAL_CONCEPTS) map[c] = "denied";
  map["variable_binding"] = "granted";
  map["string_format"] = "defer";
  const weight = recalculateWeight(map);
  // Sum: 1 + 0 + 9*(-1) = -8. |avg| = |-8/11| = 8/11 = 0.7272...
  assertAlmostEquals(weight, 8 / 11, 0.01);
});

Deno.test("data/registries: recalculateWeight — 10G 1 defer = 0.91", () => {
  // Constructors/helpers: 10 granted, self_access=defer
  const map: Record<string, TernaryValue> = {};
  for (const c of CANONICAL_CONCEPTS) map[c] = "granted";
  map["self_access"] = "defer";
  const weight = recalculateWeight(map);
  // Sum: 10 + 0 = 10. |avg| = 10/11 = 0.9090...
  assertAlmostEquals(weight, 10 / 11, 0.01);
});
...
Deno.test("data/registries/G1: every section has exactly 11 concept_map keys", () => {
...
Deno.test("data/registries/G2: every section uses the canonical concept keys", () => {
...
Deno.test("data/registries/G5: transpose consistency — 462 ternary values match", () => {
...
Deno.test("data/registries/G10: concept count is 11", () => {
  assertEquals(Object.keys(CONCEPT_REGISTRY).length, TOTAL_CONCEPTS);
});

Deno.test("data/registries/G10: concept order has 11 entries", () => {
