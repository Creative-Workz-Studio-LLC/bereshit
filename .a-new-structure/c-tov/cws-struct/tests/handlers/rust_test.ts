...
Deno.test("body/subtype-subsections: RO-acknowledged absent subsections are not reported", async () => {
...
Deno.test("policy/growth-required: required-missing becomes info under growth", async () => {
  ...
  try {
    ...
    // Growth: warn → info (one step toward info)
...
Deno.test("policy/strict-required: required-missing becomes error under strict", async () => {
  ...
  try {
    ...
    const missing = byRule(results, "form/required-section-missing");
    assertGreater(missing.length, 0, "Should still detect missing required section");
    // Strict: warn → error (one step toward error)
...
Deno.test("policy/balanced-required: required-missing stays warn under balanced", async () => {
...
Deno.test("policy/growth-reserved: reserved-present becomes info under growth", async () => {
  setGlobalPolicy("growth");
  try {
    const f = fixture("rust/form/module-has-reserved-section.rs");
    const results = await rust.lint(f);
...
Deno.test("policy/strict-reserved: reserved-present becomes error under strict", async () => {
  setGlobalPolicy("strict");
  try {
    const f = fixture("rust/form/module-has-reserved-section.rs");
    const results = await rust.lint(f);
