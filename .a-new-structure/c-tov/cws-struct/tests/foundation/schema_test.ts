...
Deno.test("loadRules: returns DerivedRules object", async () => {
  clearCache();
...
Deno.test("loadRules: contract has metadata/content/closing", async () => {
  const rules = await loadRules();
...
Deno.test("loadRules: metadataKeyPattern is a RegExp", async () => {
  const rules = await loadRules();
  assert(rules.metadataKeyPattern instanceof RegExp, "Should be a RegExp");
});

Deno.test("loadRules: closingKeyPattern is a RegExp", async () => {
  const rules = await loadRules();
  assert(rules.closingKeyPattern instanceof RegExp, "Should be a RegExp");
});

Deno.test("loadRules: metadataKeyPattern matches valid section names", async () => {
  const rules = await loadRules();
...
Deno.test("loadRules: requiredIdentity contains stripped section names", async () => {
  const rules = await loadRules();
...
Deno.test("loadRules: requiredContext contains stripped section names", async () => {
  const rules = await loadRules();
...
Deno.test("loadRules: requiredClosing contains stripped section names", async () => {
  const rules = await loadRules();
...
Deno.test("loadRules: allKnownSections is a Set", async () => {
  const rules = await loadRules();
...
Deno.test("loadRules: allKnownClosing is a Set", async () => {
  const rules = await loadRules();
...
Deno.test("loadRules: metadataFields has I1_core requirements", async () => {
  const rules = await loadRules();
...
Deno.test("loadRules: closingFields has X1_policy requirements", async () => {
  const rules = await loadRules();
...
Deno.test("clearCache: forces re-load on next call", async () => {
  // Load once (caches)
...
Deno.test("clearCache: idempotent (calling twice doesn't break)", () => {
  clearCache();
...
Deno.test("loadRules: returns same reference on second call (caching)", async () => {
  clearCache();
  const first = await loadRules();
  const second = await loadRules();
