...
// ============================================================================
//
// file:    tests/foundation/pipeline_test.ts
...
Deno.test("pipeline/default: resolves Go schema through default pipeline", async () => {
  clearPipeline();
  const pipeline = getDefaultPipeline();
  const text = await pipeline.getText("code/format/go-4block-schema.jsonc");
  assert(text.length > 0, "Should return non-empty text");
  assert(text.includes("go_structure"), "Should contain go_structure key");
});

Deno.test("pipeline/default: resolves Rust schema through default pipeline", async () => {
  clearPipeline();
  const pipeline = getDefaultPipeline();
  const text = await pipeline.getText("code/format/rust-4block-schema.jsonc");
  assert(text.length > 0, "Should return non-empty text");
...
// ---------------------------------------------------------------------------
// parseCodeSchema — pure parse (Phase 1)
// ---------------------------------------------------------------------------

Deno.test("pipeline/parseCodeSchema: parses Go schema text into rules", async () => {
  clearPipeline();
  const pipeline = getDefaultPipeline();
  const text = await pipeline.getText("code/format/go-4block-schema.jsonc");
  const rules = parseCodeSchema("go", text);
...
Deno.test("pipeline/parseCodeSchema: produces same rules as loadCodeRules", async () => {
  clearCodeCache("go");
  clearPipeline();

  // Via loadCodeRules (I/O + parse + cache)
  const loaded = await loadCodeRules("go");

  // Via pipeline + parseCodeSchema (manual)
  const pipeline = getDefaultPipeline();
  const text = await pipeline.getText("code/format/go-4block-schema.jsonc");
  const parsed = parseCodeSchema("go", text);

  // Same structure
  assertEquals(loaded.setupSubsections.length, parsed.setupSubsections.length);
  assertEquals(
    Object.keys(loaded.bodySubsections).sort(),
    Object.keys(parsed.bodySubsections).sort(),
  );
  assertEquals(
    Object.keys(loaded.pragmaFieldRequirements).sort(),
    Object.keys(parsed.pragmaFieldRequirements).sort(),
  );
});

Deno.test("pipeline/parseCodeSchema: throws on invalid JSONC", () => {
  try {
    parseCodeSchema("go", "not valid jsonc {{{");
...
Deno.test("pipeline/parseCodeSchema: throws on missing structure key", () => {
  try {
    parseCodeSchema("go", '{"unrelated": true}');
    assert(false, "Should have thrown");
  } catch (e) {
    assertStringIncludes((e as Error).message, "missing structure key");
  }
});
...
Deno.test("pipeline/isolation: custom source failure for one schema doesn't affect another", async () => {
  ...
  // Rust resolves fine
  const rustText = await pipeline.getText("code/format/rust-4block-schema.jsonc");
  ...
  const err = await assertRejects(
    () => pipeline.getText("code/format/go-4block-schema.jsonc"),
    Error,
  );
