...
Deno.test("[CWS-T00-070] errors/G1: catalog is non-empty", () => {
...
Deno.test("errors/G7: matchRule resolves dynamic directive rule", () => {
  // "directive/{{directive}}/required" should match "directive/omni:key/required"
  const entry = matchRule("directive/omni:key/required");
  assert(entry !== undefined, "directive/omni:key/required should match dynamic pattern");
  assertEquals(entry!.layerName, "R50");
  assertEquals(entry!.code, "CWS-R50-011");
});
