...
// Migrated from: tests/foundation/code_schema_test.ts
// ============================================================================
// METADATA
// ============================================================================
//
// file:    tests/T00/R10/foundation/code_schema_test.ts
...
Deno.test("[CWS-T00-093] code-schema/go: loads pragma field requirements (I1-I4)", async () => {
  clearCodeCache("go");
...
Deno.test("code-schema/rust: loads subtype definitions with Rust-specific fields", async () => {
  ...
  // 4 subtypes loaded from schema files — bare-bone is a form (FORM_REGISTRY)
  // but not a subtype definition (no schema file at subtypes/bare-bone/rust.jsonc).
  assertEquals(
    Object.keys(rules.subtypeDefinitions).sort(),
    ["demo-test", "executable", "library", "module"],
    "Rust should have 4 subtypes (library, executable, module, demo-test)",
  );
...
// NOTE: emphasis tests depend on form structure schemas (code/forms/{form}/_structure.jsonc)
// which don't exist yet. Both Go and Rust now share FORM_REGISTRY, so emphasis
// is loaded per-form (not per-format). Ignored until form structure schemas are created.

Deno.test({ name: "code-schema/equivalence: Go subtypes are subset of Rust emphasis keys", ignore: true, fn: async () => {
  const goRules = await loadCodeRules("go");
  const rustRules = await loadCodeRules("rust");
...
Deno.test({ name: "code-schema/equivalence: emphasis subtypes have heavy and light arrays", ignore: true, fn: async () => {
  const goRules = await loadCodeRules("go");
  const rustRules = await loadCodeRules("rust");
...
Deno.test("code-schema/equivalence: Go and Rust setupData have requiredIn fields", async () => {
  ...
  // S1 Imports is ALL_REQUIRED — required in all 5 forms (bare-bone through demo-test).
  // Both formats share SECTION_REGISTRY, so requiredIn covers all forms.
  ...
  assertEquals(
    goImports!.requiredIn.sort(),
    ["bare-bone", "demo-test", "executable", "library", "module"],
    "Go Imports required in all 5 forms",
  );
  assertEquals(
    rustImports!.requiredIn.sort(),
    ["bare-bone", "demo-test", "executable", "library", "module"],
    "Rust Imports required in all 5 forms",
  );
});

Deno.test("code-schema/equivalence: Go and Rust bodyData share FORM_REGISTRY keys", async () => {
  const goRules = await loadCodeRules("go");
  const rustRules = await loadCodeRules("rust");
  ...
  // Both formats derive bodyData from shared FORM_REGISTRY — same 5 form keys.
  assertEquals(
    goBodyKeys,
    ["bare-bone", "demo-test", "executable", "library", "module"],
    "Go bodyData should have all 5 FORM_REGISTRY keys",
  );
  assertEquals(
    goBodyKeys,
    rustBodyKeys,
    "Go and Rust bodyData should have identical form keys (shared FORM_REGISTRY)",
  );

  // Each form should have subsections
...
// ---------------------------------------------------------------------------
// Typing maps — arrow refinement (R[5] specificity)
// ---------------------------------------------------------------------------

Deno.test("code-schema/rust: loads typing maps for module form", async () => {
  clearCodeCache("rust");
  const rules = await loadCodeRules("rust");

  assert(rules.typingMaps !== undefined, "Should have typingMaps");
  assert(rules.typingMaps["module"] !== undefined, "Should have module typings");

  const moduleTypings = rules.typingMaps["module"]!;
  const typingNames = Object.keys(moduleTypings);
  assertGreater(typingNames.length, 0, "Should have at least one module typing");
  assert(typingNames.includes("utility"), "Should have utility typing");
  assert(typingNames.includes("service"), "Should have service typing");
  assert(typingNames.includes("parser"), "Should have parser typing");
});

Deno.test("code-schema/rust: utility typing has correct BODY profile", async () => {
  clearCodeCache("rust");
  const rules = await loadCodeRules("rust");

  const utility = rules.typingMaps["module"]!["utility"]!;
  assert(utility !== undefined, "utility typing should exist");

  // Utility modules: FreeFunctions required, type lifecycle irrelevant
  assert(utility.BODY.required.includes("Free Functions"),
    "utility BODY should require Free Functions");
  assert(utility.BODY.irrelevant.includes("Trait Implementations"),
    "utility BODY should mark Trait Implementations as irrelevant");
  assert(utility.BODY.irrelevant.includes("Constructors"),
    "utility BODY should mark Constructors as irrelevant");
  assert(utility.BODY.irrelevant.includes("Queries"),
    "utility BODY should mark Queries as irrelevant");
});

Deno.test("code-schema/rust: service typing has no irrelevant BODY sections", async () => {
  clearCodeCache("rust");
  const rules = await loadCodeRules("rust");

  const service = rules.typingMaps["module"]!["service"]!;
  assert(service !== undefined, "service typing should exist");

  assertEquals(service.BODY.irrelevant.length, 0,
    "service typing should have no irrelevant BODY sections (full lifecycle)");
  assertGreater(service.BODY.required.length, 5,
    "service typing should require most BODY sections");
});

Deno.test("code-schema/rust: utility SETUP profile narrows requirements", async () => {
  clearCodeCache("rust");
  const rules = await loadCodeRules("rust");

  const utility = rules.typingMaps["module"]!["utility"]!;

  // Utility SETUP: only Imports required, types-related sections irrelevant
  assert(utility.SETUP.required.includes("Imports"),
    "utility SETUP should require Imports");
  assert(utility.SETUP.irrelevant.includes("Core Types"),
    "utility SETUP should mark Core Types as irrelevant");
  assert(utility.SETUP.irrelevant.includes("Trait Defs"),
    "utility SETUP should mark Trait Defs as irrelevant");
});

Deno.test("code-schema/go: loads typing maps (same schema, same typing data)", async () => {
  clearCodeCache("go");
  const rules = await loadCodeRules("go");

  assert(rules.typingMaps !== undefined, "Should have typingMaps");
  // Module typings come from the form schema (format-agnostic), so Go gets them too
  assert(rules.typingMaps["module"] !== undefined, "Go should also have module typings");
  const typingNames = Object.keys(rules.typingMaps["module"]!);
  assert(typingNames.includes("utility"), "Go module typings should include utility");
});
...
// CLOSING containerization, fill content enrichment, and typing maps from
// Go and Rust schemas. The I/C standard is format-agnostic — both schemas
// define the same sections. What differs is the carrier syntax and
// format-specific features. Typing maps are format-agnostic — they come
// from the form schema, not the format schema.
//
