...
// purpose: Core type definitions for the data layer — the compiled expression
//          of schema truth. These types define WHAT handlers consume, not HOW
//          schemas are loaded. The data layer sits between schemas (design-time)
//          and handlers (runtime). Schema-driven, not schema-dependent.
//
//          42 section entries (R[25] containers)
//          11 concept entries (R[5] patterns)
//          5 form definitions (bare-bone through demo-test)
//          2 language placement maps (Go, Rust — growing)
//          7 Hebrew states (broken through perfect)
//          Error code catalog (CWS-{LAYER}-{SEQ})
//
...
import type { HebrewState, HebrewDirection } from "../foundation/mod.ts";
...
// ---------------------------------------------------------------------------
// Block Name — the 4 structural blocks
// ---------------------------------------------------------------------------

/**
 * The 4 block names in the OmniCode 4-block structure.
 *
 * Union type — not a string. Catches typos at compile time.
 * Schemas use UPPERCASE ("METADATA"), TypeScript uses lowercase ("metadata").
 */
...
// (No body logic — this file is pure type definitions and validation helpers.
//  The registries that populate these types live in their own files.)
...
// Schema-driven, not schema-dependent.
//
// "The words of the LORD are pure words: as silver tried in a furnace
//  of earth, purified seven times." — Psalm 12:6
// ============================================================================
