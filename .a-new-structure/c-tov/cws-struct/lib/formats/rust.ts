...
import type {
  FormatHandler, LintResult, TransformOptions,
 ...
 * Keys match the section prefix used in statics (e.g., "I1" not "I1_core").
 */
...
function findBlocks(lines: string[]): BlockPosition[] {
  ...
  // Sort by line number so blocks appear in file order, not declaration order.
  // This enables block order detection (e.g., SETUP before METADATA is an error).
...
/** A parsed field from a PRAGMA or METADATA static initializer. */
...
// ---------------------------------------------------------------------------
// Check 13: CLOSING zone ordering
// ---------------------------------------------------------------------------

/**
 * CLOSING zone markers — code zones and documentation sections.
 *
 * Code zones must appear BEFORE documentation sections.
 * Within each group, the ordering is canonical:
 *   Code:  Cv (validation/tests) → Ce (execution/entry) → Cc (cleanup/Drop)
 *   Docs:  X1 (policy) → X2 (extension) → X3 (troubleshooting)
 *          → X4 (reference) → X5 (closing note) → X6 (template guide)
 *
 * Patterns match the comment markers used in seed templates:
 *   Code:  "// Cv — Closing Validation"
 *   Docs:  "// X1: Modification Policy"
 */
const CLOSING_ZONES = [
 ...
 */
function checkClosingZoneOrder(ctx: RustFileContext): LintResult[] {
  const results: LintResult[] = [];
  const file = ctx.filePath;
  ...
  // Check 3: Within documentation sections, verify canonical order (X1 → X2 → ... → X6)
...
/** Range of content lines within a block (0-based indices into file lines). */
...
function findBlockRange(lines: string[], blockName: string): BlockRange | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed === `// ${blockName}` && headerLine < 0) {
      // Verify it's between = separators (look for = separator within 2 lines)
  ...
  // Skip the closing separator line of the block header
  ...
  if (endLine >= 0) {
    contentEnd = endLine;
    // Back up past the opening = separator of the END section
