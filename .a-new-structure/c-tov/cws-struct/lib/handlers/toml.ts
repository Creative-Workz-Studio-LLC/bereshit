...
// ============================================================================
//
// file:    lib/handlers/toml.ts
// key:     B-tov-cws-struct-lib-handlers-toml
...
// purpose: Schema-driven TOML 3-block alignment linter + transformer with
//          value-level validation. Validates TOML data files for:
//          - 3-block structure (_metadata → _content → _closing)
//          - Pragma line + format field consistency
//          - Required tables and subtables per schema
//          - I/C field requirements (I1-I4 PRAGMA, C1-C7 METADATA)
//          - Contract validation (required_identity, format_rules)
//          - Value-level: version format, dates, key prefix, filename match
//          - Value-level: type values, empty fields, cross-field consistency
//          - X-section ordering in _closing (X1 → X5)
//          - Phantom section detection (contract vs reality)
//
//          Schema-driven — rules loaded from toml-3block-schema.jsonc at
//          runtime. The schema is the single source of truth. Code interprets,
//          schema defines.
//
//          Schema triangle: toml-3block-schema.jsonc defines, toml.ts validates.
//
 ...
 */
export function parsePragmaLine(line: string): PragmaInfo | null {
 ...
 */
export function parseFormatField(format: string): PragmaInfo | null {
 ...
 */
export function normalizeCargoData(data: TomlData): TomlData {
...
export function has(obj: TomlData, key: string): boolean {
  return obj != null && typeof obj === "object" && key in obj;
}

export function getTable(obj: TomlData, key: string): TomlData | undefined {
  const val = obj?.[key];
 ...
 */
export function resolveTable(obj: TomlData, path: string): TomlData | undefined {
 ...
 */
export function findSectionPositions(text: string): Map<string, number> {
