...
// Key: B-word-work-pkg-trit-dimension-test
// ═══════════════════════════════════════════════════════════════════════════
//
...
int test_trit_space(void) {
    print_header("SPACE Layer Unit Tests: dimension.c");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 1: trit_to_dimension - all 3 mappings
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit_to_dimension:\n");

    // -1 → DIM_MATERIAL ("the earth")
    test_assert(trit_to_dimension(TRIT_NEG) == DIM_MATERIAL,
                "trit_to_dimension(-1) == DIM_MATERIAL");

    // 0 → DIM_TEMPORAL ("in the beginning")
    test_assert(trit_to_dimension(TRIT_ZERO) == DIM_TEMPORAL,
                "trit_to_dimension(0) == DIM_TEMPORAL");

    // +1 → DIM_SPATIAL ("the heavens")
    test_assert(trit_to_dimension(TRIT_POS) == DIM_SPATIAL,
                "trit_to_dimension(+1) == DIM_SPATIAL");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 2: dimension_to_trit - all 3 mappings (inverse)
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing dimension_to_trit:\n");

    // DIM_MATERIAL → -1
    test_assert(trit_value(dimension_to_trit(DIM_MATERIAL)) == -1,
                "dimension_to_trit(DIM_MATERIAL) == -1");

    // DIM_TEMPORAL → 0
    test_assert(trit_value(dimension_to_trit(DIM_TEMPORAL)) == 0,
                "dimension_to_trit(DIM_TEMPORAL) == 0");

    // DIM_SPATIAL → +1
    test_assert(trit_value(dimension_to_trit(DIM_SPATIAL)) == +1,
                "dimension_to_trit(DIM_SPATIAL) == +1");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 3: Roundtrip - trit → dimension → trit
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing roundtrip (trit → dimension → trit):\n");

    trit_t trits[3] = {TRIT_NEG, TRIT_ZERO, TRIT_POS};
    int roundtrip_ok = 1;
    for (int i = 0; i < 3; i++) {
        dimension_t dim = trit_to_dimension(trits[i]);
        trit_t back = dimension_to_trit(dim);
        if (trit_value(back) != trit_value(trits[i])) {
            roundtrip_ok = 0;
        }
    }
    test_assert(roundtrip_ok, "roundtrip: all 3 trits preserved through dimension");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 4: trit_navigate - building block navigation
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit_navigate:\n");

    // From anchor (0), build up → +1
    test_assert(trit_value(trit_navigate(TRIT_ZERO, DIR_BUILD_UP)) == +1,
                "navigate(0, BUILD_UP) == +1");

    // From anchor (0), break down → -1
    test_assert(trit_value(trit_navigate(TRIT_ZERO, DIR_BREAK_DOWN)) == -1,
                "navigate(0, BREAK_DOWN) == -1");

    // From anchor (0), stay → 0
    test_assert(trit_value(trit_navigate(TRIT_ZERO, DIR_ANCHOR)) == 0,
                "navigate(0, ANCHOR) == 0");

    // Edge: from +1, build up → +1 (clamped at boundary)
    test_assert(trit_value(trit_navigate(TRIT_POS, DIR_BUILD_UP)) == +1,
                "navigate(+1, BUILD_UP) == +1 (clamped)");

    // Edge: from -1, break down → -1 (clamped at boundary)
    test_assert(trit_value(trit_navigate(TRIT_NEG, DIR_BREAK_DOWN)) == -1,
                "navigate(-1, BREAK_DOWN) == -1 (clamped)");

    // From +1, break down → 0
    test_assert(trit_value(trit_navigate(TRIT_POS, DIR_BREAK_DOWN)) == 0,
                "navigate(+1, BREAK_DOWN) == 0");

    // From -1, build up → 0
    test_assert(trit_value(trit_navigate(TRIT_NEG, DIR_BUILD_UP)) == 0,
                "navigate(-1, BUILD_UP) == 0");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 5: dimension_path - all 9 combinations
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing dimension_path (all 9 combinations):\n");

    // Same dimension → ANCHOR (stay put)
    test_assert(dimension_path(DIM_MATERIAL, DIM_MATERIAL) == DIR_ANCHOR,
                "path(MATERIAL, MATERIAL) == ANCHOR");
    test_assert(dimension_path(DIM_TEMPORAL, DIM_TEMPORAL) == DIR_ANCHOR,
                "path(TEMPORAL, TEMPORAL) == ANCHOR");
    test_assert(dimension_path(DIM_SPATIAL, DIM_SPATIAL) == DIR_ANCHOR,
                "path(SPATIAL, SPATIAL) == ANCHOR");

    // Upward paths → BUILD_UP
    test_assert(dimension_path(DIM_MATERIAL, DIM_TEMPORAL) == DIR_BUILD_UP,
                "path(MATERIAL, TEMPORAL) == BUILD_UP");
    test_assert(dimension_path(DIM_MATERIAL, DIM_SPATIAL) == DIR_BUILD_UP,
                "path(MATERIAL, SPATIAL) == BUILD_UP");
    test_assert(dimension_path(DIM_TEMPORAL, DIM_SPATIAL) == DIR_BUILD_UP,
                "path(TEMPORAL, SPATIAL) == BUILD_UP");

    // Downward paths → BREAK_DOWN
    test_assert(dimension_path(DIM_SPATIAL, DIM_TEMPORAL) == DIR_BREAK_DOWN,
                "path(SPATIAL, TEMPORAL) == BREAK_DOWN");
    test_assert(dimension_path(DIM_SPATIAL, DIM_MATERIAL) == DIR_BREAK_DOWN,
                "path(SPATIAL, MATERIAL) == BREAK_DOWN");
    test_assert(dimension_path(DIM_TEMPORAL, DIM_MATERIAL) == DIR_BREAK_DOWN,
                "path(TEMPORAL, MATERIAL) == BREAK_DOWN");
...
int test_trit_run_all(void) {
    ...
    test_trit_space();        // SPACE: dimension mapping and navigation
