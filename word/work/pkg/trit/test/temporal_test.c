...
// Key: B-word-work-pkg-trit-temporal-test
// ═══════════════════════════════════════════════════════════════════════════
//
...
int test_trit_time(void) {
    print_header("TIME Layer Unit Tests: temporal.c");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 1: trit_to_temporal - all 3 mappings
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit_to_temporal:\n");

    // -1 → TEMP_PAST (what was)
    test_assert(trit_to_temporal(TRIT_NEG) == TEMP_PAST,
                "trit_to_temporal(-1) == TEMP_PAST");

    // 0 → TEMP_PRESENT (what is)
    test_assert(trit_to_temporal(TRIT_ZERO) == TEMP_PRESENT,
                "trit_to_temporal(0) == TEMP_PRESENT");

    // +1 → TEMP_FUTURE (what will be)
    test_assert(trit_to_temporal(TRIT_POS) == TEMP_FUTURE,
                "trit_to_temporal(+1) == TEMP_FUTURE");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 2: temporal_to_trit - all 3 mappings (inverse)
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing temporal_to_trit:\n");

    // TEMP_PAST → -1
    test_assert(trit_value(temporal_to_trit(TEMP_PAST)) == -1,
                "temporal_to_trit(TEMP_PAST) == -1");

    // TEMP_PRESENT → 0
    test_assert(trit_value(temporal_to_trit(TEMP_PRESENT)) == 0,
                "temporal_to_trit(TEMP_PRESENT) == 0");

    // TEMP_FUTURE → +1
    test_assert(trit_value(temporal_to_trit(TEMP_FUTURE)) == +1,
                "temporal_to_trit(TEMP_FUTURE) == +1");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 3: Roundtrip - trit → temporal → trit
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing roundtrip (trit → temporal → trit):\n");

    trit_t trits[3] = {TRIT_NEG, TRIT_ZERO, TRIT_POS};
    int roundtrip_ok = 1;
    for (int i = 0; i < 3; i++) {
        temporal_t temp = trit_to_temporal(trits[i]);
        trit_t back = temporal_to_trit(temp);
        if (trit_value(back) != trit_value(trits[i])) {
            roundtrip_ok = 0;
        }
    }
    test_assert(roundtrip_ok, "roundtrip: all 3 trits preserved through temporal");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 4: temporal_create_base - 3 base states
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing temporal_create_base:\n");

    // Base PAST: lens == target == PAST
    temporal_state_t base_past = temporal_create_base(TEMP_PAST);
    test_assert(base_past.lens == TEMP_PAST && base_past.target == TEMP_PAST,
                "create_base(PAST): lens == target == PAST");

    // Base PRESENT: lens == target == PRESENT
    temporal_state_t base_present = temporal_create_base(TEMP_PRESENT);
    test_assert(base_present.lens == TEMP_PRESENT && base_present.target == TEMP_PRESENT,
                "create_base(PRESENT): lens == target == PRESENT");

    // Base FUTURE: lens == target == FUTURE
    temporal_state_t base_future = temporal_create_base(TEMP_FUTURE);
    test_assert(base_future.lens == TEMP_FUTURE && base_future.target == TEMP_FUTURE,
                "create_base(FUTURE): lens == target == FUTURE");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 5: temporal_is_compound - base states are NOT compound
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing temporal_is_compound (base states):\n");

    test_assert(!temporal_is_compound(base_past),
                "is_compound(base PAST) == false");
    test_assert(!temporal_is_compound(base_present),
                "is_compound(base PRESENT) == false");
    test_assert(!temporal_is_compound(base_future),
                "is_compound(base FUTURE) == false");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 6: temporal_create_compound - all 6 compound states
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing temporal_create_compound (all 6 modes):\n");

    // PAST → PRESENT (Integration)
    temporal_state_t past_present = temporal_create_compound(TEMP_PAST, TEMP_PRESENT);
    test_assert(past_present.lens == TEMP_PAST && past_present.target == TEMP_PRESENT,
                "compound(PAST, PRESENT): Integration");

    // PAST → FUTURE (Prophecy)
    temporal_state_t past_future = temporal_create_compound(TEMP_PAST, TEMP_FUTURE);
    test_assert(past_future.lens == TEMP_PAST && past_future.target == TEMP_FUTURE,
                "compound(PAST, FUTURE): Prophecy");

    // PRESENT → PAST (Learning)
    temporal_state_t present_past = temporal_create_compound(TEMP_PRESENT, TEMP_PAST);
    test_assert(present_past.lens == TEMP_PRESENT && present_past.target == TEMP_PAST,
                "compound(PRESENT, PAST): Learning");

    // PRESENT → FUTURE (Planning)
    temporal_state_t present_future = temporal_create_compound(TEMP_PRESENT, TEMP_FUTURE);
    test_assert(present_future.lens == TEMP_PRESENT && present_future.target == TEMP_FUTURE,
                "compound(PRESENT, FUTURE): Planning");

    // FUTURE → PAST (Vision)
    temporal_state_t future_past = temporal_create_compound(TEMP_FUTURE, TEMP_PAST);
    test_assert(future_past.lens == TEMP_FUTURE && future_past.target == TEMP_PAST,
                "compound(FUTURE, PAST): Vision");

    // FUTURE → PRESENT (Preparation)
    temporal_state_t future_present = temporal_create_compound(TEMP_FUTURE, TEMP_PRESENT);
    test_assert(future_present.lens == TEMP_FUTURE && future_present.target == TEMP_PRESENT,
                "compound(FUTURE, PRESENT): Preparation");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 7: temporal_is_compound - compound states ARE compound
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing temporal_is_compound (compound states):\n");

    test_assert(temporal_is_compound(past_present),
                "is_compound(PAST→PRESENT) == true");
    test_assert(temporal_is_compound(past_future),
                "is_compound(PAST→FUTURE) == true");
    test_assert(temporal_is_compound(present_past),
                "is_compound(PRESENT→PAST) == true");
    test_assert(temporal_is_compound(present_future),
                "is_compound(PRESENT→FUTURE) == true");
    test_assert(temporal_is_compound(future_past),
                "is_compound(FUTURE→PAST) == true");
    test_assert(temporal_is_compound(future_present),
                "is_compound(FUTURE→PRESENT) == true");
...
int test_trit_run_all(void) {
    ...
    test_trit_time();         // TIME: temporal states, 9 cognitive modes
