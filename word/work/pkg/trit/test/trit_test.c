...
// Key: B-word-work-pkg-trit-trit-test
// ═══════════════════════════════════════════════════════════════════════════
//
...
int test_trit_matter(void) {
    print_header("MATTER Layer Unit Tests: trit.c");

    // ─────────────────────────────────────────────────────────────
    // TEST GROUP 1: trit_create - valid inputs
    // ─────────────────────────────────────────────────────────────
    printf("\n  trit_create() - valid inputs:\n");

    trit_t neg  = trit_create(-1);
    trit_t zero = trit_create(0);
    trit_t pos  = trit_create(+1);

    test_assert(trit_valid(neg),  "trit_create(-1) produces valid trit");
    test_assert(trit_valid(zero), "trit_create(0) produces valid trit");
    test_assert(trit_valid(pos),  "trit_create(+1) produces valid trit");

    // ─────────────────────────────────────────────────────────────
    // TEST GROUP 2: trit_value - extraction
    // ─────────────────────────────────────────────────────────────
    printf("\n  trit_value() - value extraction:\n");

    test_assert(trit_value(neg) == -1,  "trit_value(neg) == -1");
    test_assert(trit_value(zero) == 0,  "trit_value(zero) == 0");
    test_assert(trit_value(pos) == +1,  "trit_value(pos) == +1");

    // ─────────────────────────────────────────────────────────────
    // TEST GROUP 3: trit_negate - all cases
    // ─────────────────────────────────────────────────────────────
    printf("\n  trit_negate() - direction reversal:\n");

    test_assert(trit_value(trit_negate(neg)) == +1,  "negate(-1) == +1");
    test_assert(trit_value(trit_negate(zero)) == 0,  "negate(0) == 0 (anchor fixed)");
    test_assert(trit_value(trit_negate(pos)) == -1,  "negate(+1) == -1");

    // ─────────────────────────────────────────────────────────────
    // TEST GROUP 4: trit_add - full 3x3 table (from ternary-math.toml)
    // ─────────────────────────────────────────────────────────────
    printf("\n  trit_add() - full addition table:\n");

    // Row -1: [-1+-1, -1+0, -1+1]
    test_assert(trit_value(trit_add(neg, neg)) == -1,  "(-1) + (-1) == -1 (saturate)");
    test_assert(trit_value(trit_add(neg, zero)) == -1, "(-1) + (0) == -1");
    test_assert(trit_value(trit_add(neg, pos)) == 0,   "(-1) + (+1) == 0 (cancel)");

    // Row 0: [0+-1, 0+0, 0+1]
    test_assert(trit_value(trit_add(zero, neg)) == -1, "(0) + (-1) == -1");
    test_assert(trit_value(trit_add(zero, zero)) == 0, "(0) + (0) == 0");
    test_assert(trit_value(trit_add(zero, pos)) == +1, "(0) + (+1) == +1");

    // Row +1: [1+-1, 1+0, 1+1]
    test_assert(trit_value(trit_add(pos, neg)) == 0,   "(+1) + (-1) == 0 (cancel)");
    test_assert(trit_value(trit_add(pos, zero)) == +1, "(+1) + (0) == +1");
    test_assert(trit_value(trit_add(pos, pos)) == +1,  "(+1) + (+1) == +1 (saturate)");

    // ─────────────────────────────────────────────────────────────
    // TEST GROUP 5: trit_multiply - full 3x3 table (from ternary-math.toml)
    // ─────────────────────────────────────────────────────────────
    printf("\n  trit_multiply() - full multiplication table:\n");

    // Row -1: [-1*-1, -1*0, -1*1]
    test_assert(trit_value(trit_multiply(neg, neg)) == +1,  "(-1) * (-1) == +1");
    test_assert(trit_value(trit_multiply(neg, zero)) == 0,  "(-1) * (0) == 0");
    test_assert(trit_value(trit_multiply(neg, pos)) == -1,  "(-1) * (+1) == -1");

    // Row 0: [0*-1, 0*0, 0*1]
    test_assert(trit_value(trit_multiply(zero, neg)) == 0,  "(0) * (-1) == 0");
    test_assert(trit_value(trit_multiply(zero, zero)) == 0, "(0) * (0) == 0");
    test_assert(trit_value(trit_multiply(zero, pos)) == 0,  "(0) * (+1) == 0");

    // Row +1: [1*-1, 1*0, 1*1]
    test_assert(trit_value(trit_multiply(pos, neg)) == -1,  "(+1) * (-1) == -1");
    test_assert(trit_value(trit_multiply(pos, zero)) == 0,  "(+1) * (0) == 0");
    test_assert(trit_value(trit_multiply(pos, pos)) == +1,  "(+1) * (+1) == +1");

    return tests_failed;  // Return failure count for diagnosis
}
...
int test_trit_run_all(void) {
    ...
    // ═══════════════════════════════════════════════════════════
    test_trit_matter();       // MATTER: trit creation, arithmetic, edge cases
    // Other layers tested in their own files:
