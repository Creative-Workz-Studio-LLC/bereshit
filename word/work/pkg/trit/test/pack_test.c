...
// Key: B-word-work-pkg-trit-pack-test
// ═══════════════════════════════════════════════════════════════════════════
//
...
int test_trit_space(void) {
    print_header("SPACE Layer Unit Tests: pack.c");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 1: trit5_pack/unpack roundtrip - ALL 243 states
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit5_pack/unpack roundtrip (all 243 states):\n");

    int roundtrip_failures = 0;
    int first_failure_state = -1;

    // Iterate all 243 states (0 to 242)
    for (int state = 0; state < 243; state++) {
        // Convert state to 5 trits manually (base-3 with offset)
        trit_t original[5];
        int temp = state;
        for (int i = 0; i < 5; i++) {
            int digit = temp % 3;
            original[i] = trit_create(digit - 1);  // 0→-1, 1→0, 2→+1
            temp /= 3;
        }
        ...
        // Verify each trit matches
        for (int i = 0; i < 5; i++) {
            if (trit_value(original[i]) != trit_value(unpacked[i])) {
                roundtrip_failures++;
                if (first_failure_state < 0) first_failure_state = state;
                break;
            }
        }
    }
    ...
    if (roundtrip_failures == 0) {
        test_assert(1, "trit5_pack/unpack roundtrip: all 243 states");
    } else {
        printf("    First failure at state %d\n", first_failure_state);
        test_assert(0, "trit5_pack/unpack roundtrip: all 243 states");
    }

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 2: trit5_pack known values
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit5_pack known values:\n");

    // State 0: [-1,-1,-1,-1,-1] (all negative)
    trit_t all_neg[5] = {TRIT_NEG, TRIT_NEG, TRIT_NEG, TRIT_NEG, TRIT_NEG};
    test_assert(trit5_pack(all_neg) == 0, "pack([-1,-1,-1,-1,-1]) == 0");

    // State 121: [0,0,0,0,0] (all zero - middle)
    trit_t all_zero[5] = {TRIT_ZERO, TRIT_ZERO, TRIT_ZERO, TRIT_ZERO, TRIT_ZERO};
    test_assert(trit5_pack(all_zero) == 121, "pack([0,0,0,0,0]) == 121 (middle)");

    // State 242: [+1,+1,+1,+1,+1] (all positive)
    trit_t all_pos[5] = {TRIT_POS, TRIT_POS, TRIT_POS, TRIT_POS, TRIT_POS};
    test_assert(trit5_pack(all_pos) == 242, "pack([+1,+1,+1,+1,+1]) == 242 (max)");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 3: trit5_unpack known values
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit5_unpack known values:\n");

    trit_t result[5];

    // Unpack 0 → all negative
    trit5_unpack(0, result);
    int all_neg_ok = 1;
    for (int i = 0; i < 5; i++) {
        if (trit_value(result[i]) != -1) all_neg_ok = 0;
    }
    test_assert(all_neg_ok, "unpack(0) → [-1,-1,-1,-1,-1]");

    // Unpack 121 → all zero
    trit5_unpack(121, result);
    int all_zero_ok = 1;
    for (int i = 0; i < 5; i++) {
        if (trit_value(result[i]) != 0) all_zero_ok = 0;
    }
    test_assert(all_zero_ok, "unpack(121) → [0,0,0,0,0]");

    // Unpack 242 → all positive
    trit5_unpack(242, result);
    int all_pos_ok = 1;
    for (int i = 0; i < 5; i++) {
        if (trit_value(result[i]) != 1) all_pos_ok = 0;
    }
    test_assert(all_pos_ok, "unpack(242) → [+1,+1,+1,+1,+1]");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 4: trit9_pack/unpack
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit9_pack/unpack:\n");

    // Middle value: all zeros → 9841 (3^9/2 = 19683/2 ≈ 9841)
    trit_t nine_zeros[9] = {TRIT_ZERO, TRIT_ZERO, TRIT_ZERO, TRIT_ZERO, TRIT_ZERO,
                           TRIT_ZERO, TRIT_ZERO, TRIT_ZERO, TRIT_ZERO};
    trit9_t packed9 = trit9_pack(nine_zeros);
    test_assert(packed9 == 9841, "trit9_pack([0,0,0,0,0,0,0,0,0]) == 9841");

    // Roundtrip test
    trit_t unpacked9[9];
    trit9_unpack(packed9, unpacked9);
    int nine_ok = 1;
    for (int i = 0; i < 9; i++) {
        if (trit_value(unpacked9[i]) != 0) nine_ok = 0;
    }
    test_assert(nine_ok, "trit9_unpack(9841) → all zeros");

    // Edge: all negative → 0
    trit_t nine_neg[9] = {TRIT_NEG, TRIT_NEG, TRIT_NEG, TRIT_NEG, TRIT_NEG,
                         TRIT_NEG, TRIT_NEG, TRIT_NEG, TRIT_NEG};
    test_assert(trit9_pack(nine_neg) == 0, "trit9_pack(all -1) == 0");

    // Edge: all positive → 19682
    trit_t nine_pos[9] = {TRIT_POS, TRIT_POS, TRIT_POS, TRIT_POS, TRIT_POS,
                         TRIT_POS, TRIT_POS, TRIT_POS, TRIT_POS};
    test_assert(trit9_pack(nine_pos) == 19682, "trit9_pack(all +1) == 19682");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 5: trit27_pack/unpack
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit27_pack/unpack:\n");

    // All zeros → middle value
    trit_t t27_zeros[27];
    for (int i = 0; i < 27; i++) t27_zeros[i] = TRIT_ZERO;
    trit27_t packed27 = trit27_pack(t27_zeros);
    // Middle = (3^27 - 1) / 2 = 3812798742493
    test_assert(packed27 == 3812798742493ULL, "trit27_pack(all 0) == 3812798742493");

    // Roundtrip
    trit_t unpacked27[27];
    trit27_unpack(packed27, unpacked27);
    int t27_ok = 1;
    for (int i = 0; i < 27; i++) {
        if (trit_value(unpacked27[i]) != 0) t27_ok = 0;
    }
    test_assert(t27_ok, "trit27_unpack roundtrip: all zeros preserved");

    // Edge: all negative → 0
    trit_t t27_neg[27];
    for (int i = 0; i < 27; i++) t27_neg[i] = TRIT_NEG;
    test_assert(trit27_pack(t27_neg) == 0, "trit27_pack(all -1) == 0");
...
int test_trit_bible_rail(void) {
    print_header("Bible Rail Unit Tests: trit5_is_spare");

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 1: Valid states (0-242) should NOT be spare
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit5_is_spare for valid range (0-242):\n");

    int valid_failures = 0;
    int first_valid_failure = -1;

    // All 243 valid states should return false
    for (int state = 0; state < 243; state++) {
        if (trit5_is_spare((trit5_t)state)) {
            valid_failures++;
            if (first_valid_failure < 0) first_valid_failure = state;
        }
    }

    if (valid_failures == 0) {
        test_assert(1, "trit5_is_spare: all 243 valid states return false");
    } else {
        printf("    First incorrect 'spare' at state %d\n", first_valid_failure);
        test_assert(0, "trit5_is_spare: all 243 valid states return false");
    }

    // ════════════════════════════════════════════════════════════════
    // TEST GROUP 2: Spare states (243-255) SHOULD be spare
    // ════════════════════════════════════════════════════════════════
    printf("\n  Testing trit5_is_spare for spare range (243-255):\n");

    int spare_failures = 0;
    int first_spare_failure = -1;

    // All 13 spare states should return true
    for (int state = 243; state <= 255; state++) {
        if (!trit5_is_spare((trit5_t)state)) {
            spare_failures++;
            if (first_spare_failure < 0) first_spare_failure = state;
        }
    }

    if (spare_failures == 0) {
        test_assert(1, "trit5_is_spare: all 13 spare states (243-255) return true");
    } else {
        printf("    First incorrect 'valid' at state %d\n", first_spare_failure);
        test_assert(0, "trit5_is_spare: all 13 spare states (243-255) return true");
    }
    ...
    // Edge: state 0 (first valid)
    test_assert(!trit5_is_spare(0), "trit5_is_spare(0) == false (first valid)");

    // Edge: state 121 (middle valid)
    test_assert(!trit5_is_spare(121), "trit5_is_spare(121) == false (middle)");

    // Edge: state 242 (last valid)
    test_assert(!trit5_is_spare(242), "trit5_is_spare(242) == false (last valid)");

    // Edge: state 243 (first spare)
    test_assert(trit5_is_spare(243), "trit5_is_spare(243) == true (first spare)");

    // Edge: state 255 (last spare)
    test_assert(trit5_is_spare(255), "trit5_is_spare(255) == true (last spare)");
...
int test_trit_run_all(void) {
    ...
    test_trit_space();        // SPACE: pack/unpack operations
    // test_trit_time();      → temporal_test.c
    test_trit_bible_rail();   // Bible Rail: spare state detection (243-255)
    // test_trit_integration();→ integration_test.c
