...
_"Is not this laid up in store with me, and sealed up among my treasures?"_ — Deuteronomy 32:34 KJV

_"Isn't this laid up in store with me, sealed up among my treasures?"_ — Deuteronomy 32:34 WEB

'''

*<<overview,Overview>>* • *<<packing,Packing>>* • *<<unpacking,Unpacking>>* • *<<alignment,Alignment>>* • *<<layouts,Memory Layouts>>* • *<<tradeoffs,Trade-offs>>*
...
This document defines the *storage algorithms* for balanced ternary systems. While link:ternary-conversion-algorithms.adoc[Ternary Conversion Algorithms] handles translation between bases, this document specifies *how to efficiently store ternary data in binary memory* — packing trits into bytes, unpacking for computation, and optimizing for modern CPU architectures.

[IMPORTANT]
====
*5 trits per byte (t5b1) achieves 99.06% theoretical efficiency.* This is the optimal packing: 3^5 = 243 states fit into 256-state bytes, using 1.6 bits per trit (vs 1.585 theoretical minimum). This encoding is now industry standard — used by IOTA, llama.cpp (BitNet), and other ternary systems.
====
...
Verification: 65 = 0×81 + 2×27 + 1×9 + 0×3 + 2×1 = 54 + 9 + 2 = 65 ✓
----
...
Sequential method:
  65 mod 3 = 2, 65 / 3 = 21  → t0 = 2
  21 mod 3 = 0, 21 / 3 = 7   → t1 = 0
  7 mod 3 = 1, 7 / 3 = 2     → t2 = 1
  2 mod 3 = 2, 2 / 3 = 0     → t3 = 2
  0 mod 3 = 0, 0 / 3 = 0     → t4 = 0

Unsigned: [2, 0, 1, 2, 0]
Balanced: [1, -1, 0, 1, -1] ✓

SIMD method:
  65 × 3 = 195 = 0×256 + 195  → t0 = 0? No, wait...
----
...
| ~5-6 bytes packed
| 27
| Machine word (3^27 ≈ 7.6 trillion values)
|===
...
These constants extend `word/core/ternary-math.toml` with storage data.
...
[storage.efficiency]
theoretical_bits_per_trit = 1.58496  # log₂(3)
actual_bits_per_trit = 1.6           # 8 bits / 5 trits
efficiency_percent = 99.06           # 1.58496 / 1.6 × 100
----
...
cache_line_trits = 320       # 64 × 5 trits

avx2_bytes = 32              # AVX2 register width
avx2_trits = 160             # 32 × 5 trits

avx512_bytes = 64            # AVX-512 register width
avx512_trits = 320           # 64 × 5 trits
----
...
trybble_bytes = 1            # ceil(3 × 1.6 / 8)
tryte_bytes = 2              # ceil(9 × 1.6 / 8)
word_bytes = 6               # ceil(27 × 1.6 / 8)
----
...
[NOTE]
====
*Why Deuteronomy 32:34?* God describes how He stores value: "laid up in store... sealed up among my treasures." Storage algorithms do the same — they lay up trits in store (memory), sealed up (packed) among the treasures (the data structure).
====
...
| *Laid up in store*
| Trits stored in memory — organized, accessible, preserved

| *Sealed up*
| Packing algorithms — 5 trits sealed into 1 byte, compact and efficient

| *Among my treasures*
| Memory structures — the treasure house that holds the valuable data

| *With me*
| Retrievable — unpacking algorithms restore what was stored
|===

*Supporting Scripture:*

* *Malachi 3:10 KJV* — "Bring ye all the tithes into the storehouse"
* *Malachi 3:10 WEB* — "Bring the whole tithe into the storehouse"
...
| libtrit pack/unpack implementation

| 💻
| *link:../../work/pkg/trit/include/trit.h[trit.h]*
| trit5_t, trit9_t, trit27_t types for storage

| 📝
| *link:../../journey/phase-1/04-integration.adoc[Phase 1: Integration]*
| How storage research became pack.c

| 📝
| *link:ternary-packing-algorithms.adoc[Packing Algorithms]*
| Mathematical proofs for Horner's method
|===
...
_"Is not this laid up in store with me?"_ — *Deuteronomy 32:34 KJV*

_"Isn't this laid up in store with me, sealed up among my treasures?"_ — *Deuteronomy 32:34 WEB*

*Ternary data, laid up in store — sealed efficiently, retrieved faithfully.*
--
