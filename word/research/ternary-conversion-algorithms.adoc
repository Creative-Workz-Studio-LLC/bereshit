...
_"And they knew not that Joseph understood them; for he spake unto them by an interpreter."_ — Genesis 42:23 KJV

_"They didn't know that Joseph understood them; for there was an interpreter between them."_ — Genesis 42:23 WEB

'''

*<<overview,Overview>>* • *<<decimal-to-ternary,Decimal→Ternary>>* • *<<ternary-to-decimal,Ternary→Decimal>>* • *<<binary-conversions,Binary Conversions>>* • *<<intermediate,Intermediate Forms>>* • *<<derived-constants,Constants>>*
...
This document defines the *conversion algorithms* for balanced ternary systems. While link:ternary-arithmetic-algorithms.adoc[Ternary Arithmetic Algorithms] handles computation within ternary, this document specifies *translation between representations* — how the same value is expressed in different bases.

[IMPORTANT]
====
*Conversion preserves VALUE, not representation.* The integer 42 is the same whether written as "42" (decimal), "101010" (binary), or "1T-0" (balanced ternary). Conversion algorithms are interpreters — they translate between encodings while preserving meaning.
====
...
* **IN:** Decimal↔ternary, binary↔ternary, intermediate representations, optimization strategies
...
Phase 2 - Normalize:
Position 0: digit=1 → keep 1
Position 1: digit=0 → keep 0
Position 2: digit=1 → keep 1
Position 3: digit=2 → change to -1, carry 1
Position 4: carry=1 → add 1

Balanced (MSB first): 1(-1)101 or 1T101

Verification:
1×81 + (-1)×27 + 1×9 + 0×3 + 1×1
= 81 - 27 + 9 + 0 + 1
= 64 ✓
----
...
decimal = Σ(d_i × 3^i) for i from 0 to n
----
...
More efficient — avoids computing large powers:
...
From the TERNAC computerfootnote:[TERNAC: A ternary computer built at SUNY Buffalo in 1975] — encode balanced ternary as two binary words:
...
uint32_t bct_to_decimal_parallel(uint32_t bct) {
    ...
    // Stage 1: Combine adjacent trits (base 3 → base 9)
    // Each 2-bit pair: value = high_bit + 2*low_bit
    // Subtract to convert base-4 to base-3
    acc = acc - (((acc >> 2) & 0x33333333) * (4 - 3));

    // Stage 2: Combine base-9 pairs (base 9 → base 81)
    acc = acc - (((acc >> 4) & 0x0F0F0F0F) * (16 - 9));

    // Stage 3: Combine base-81 pairs (base 81 → base 6561)
...
function balancedToUnbalanced(balanced):
    // Add 1 to each digit: -1→0, 0→1, +1→2
...
Negation in balanced ternary is trivial — flip each trit:
...
These constants extend `word/constants/ternary-math.toml` with conversion data.
...
trits_1 = 1       # range: -1 to +1 → 0 to 2
trits_2 = 4       # range: -4 to +4 → 0 to 8
trits_3 = 13      # range: -13 to +13 → 0 to 26
trits_4 = 40      # range: -40 to +40 → 0 to 80
trits_5 = 121     # range: -121 to +121 → 0 to 242
trits_6 = 364     # range: -364 to +364 → 0 to 728
----
...
----
[conversion.bct]
# Jones encoding: balanced trit → 2-bit code
negative = 0b00   # -1 → 00
zero = 0b01       # 0 → 01
positive = 0b10   # +1 → 10
unused = 0b11     # not used

# Decoding: 2-bit code → balanced trit
...
| *Binary → Ternary (via decimal)*
| O(n + log₃ n)
| O(n)
| n = bits

| *Ternary → Binary (via decimal)*
| O(k + log₂ n)
| O(k)
| k = trits
...
[NOTE]
====
*Why Genesis 42:23?* Joseph understood his brothers but spoke through an interpreter. Conversion algorithms are interpreters — they translate between number system encodings while preserving the underlying value. The meaning (the number) remains constant; only the representation changes.
====
...
| *Value Preservation*
| Joseph's meaning was preserved through interpretation — conversion preserves numerical value

| *Multiple Encodings*
| Hebrew, Egyptian — same message, different languages. Binary, ternary, decimal — same value, different bases

| *Bidirectional Translation*
| The interpreter worked both ways. Conversion algorithms work both ways.

| *Interpreter Role*
| The interpreter sits "between them" — conversion algorithms sit between number systems
|===

*Supporting Scripture:*

* *Hebrews 7:2 KJV* — "First being by interpretation King of righteousness"
* *Hebrews 7:2 WEB* — "Being first, by interpretation, king of righteousness"

Note the encoding: "Melchizedek" interpreted means "king of righteousness," "Salem" interpreted means "peace." Same entity, different representations — exactly what conversion algorithms do.
...
* link:ternary-mathematical-foundations.adoc[Ternary Mathematical Foundations] — Why ternary is optimal (Paper 1)
...
_"For he spake unto them by an interpreter."_ — *Genesis 42:23 KJV*

_"There was an interpreter between them."_ — *Genesis 42:23 WEB*

*Conversion algorithms interpret between number systems — different encodings, same value.*
--
