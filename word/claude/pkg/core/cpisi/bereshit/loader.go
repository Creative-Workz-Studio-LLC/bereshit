...
// #!omni meta.key = claude-global-pkg-cpisi-bereshit-loader
...
func LoadTransliteration(dir string) (*Transliteration, error) {
	...
	if !fileExists(encPath) {
		return nil, fmt.Errorf("encryption file not found: %s", encPath)
	}

	var encData rawEncryptionFile
	if _, err := toml.DecodeFile(encPath, &encData); err != nil {
		return nil, fmt.Errorf("parse encryption.toml: %w", err)
	}
...
func MustLoadTransliteration(dir string) *Transliteration {
	...
	if err != nil {
		panic(fmt.Sprintf("bereshit.MustLoadTransliteration: %v", err))
	}
	return trans
}
