...
func loadSchemaFile[T any](path string) (*T, error) {
	...
	if _, err := toml.DecodeFile(path, &schema); err != nil {
		return nil, fmt.Errorf("schema parse error %s: %w", filepath.Base(path), err)
	}

	return &schema, nil
}

// loadJSONCFile loads a JSONC file (JSON with comments).
...
func loadJSONCFile[T any](path string) (*T, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("config read error %s: %w", filepath.Base(path), err)
	}
	...
	if err := json.Unmarshal([]byte(strings.Join(cleaned, "\n")), &result); err != nil {
		return nil, fmt.Errorf("config parse error %s: %w", filepath.Base(path), err)
	}
...
func loadTextFile(path string) (string, error) {
	...
	data, err := os.ReadFile(path)
	if err != nil {
		return "", fmt.Errorf("file read error %s: %w", filepath.Base(path), err)
	}
...
func MustLoadHalts(dir string) *HaltsSchema {
	halts, err := LoadHalts(dir)
	if err != nil {
		panic(fmt.Sprintf("schema.MustLoadHalts: %v", err))
	}
	return halts
}
...
func MustLoadInstance(instanceDir string, name string) *Instance {
	instance, err := LoadInstance(instanceDir, name)
	if err != nil {
		panic(fmt.Sprintf("schema.MustLoadInstance: %v", err))
	}
	return instance
}
...
func MustLoadUser(userDir string, name string) *User {
	user, err := LoadUser(userDir, name)
	if err != nil {
		panic(fmt.Sprintf("schema.MustLoadUser: %v", err))
	}
	return user
}
...
func MustLoadSchemas(dir string) *Schemas {
	...
	if err != nil {
		panic(fmt.Sprintf("schema.MustLoadSchemas: %v", err))
	}
	return schemas
}
