...
//	Import:   import "github.com/creativeworkzstudio/claude-global/pkg/orchestration/config"
...
// Traces back to config.toml. See: BODY > Error Helpers for constructors

// LoadError wraps file-level loading failures.
// Format: "load config.toml: parse: unexpected EOF"
type LoadError struct {
	File string // config file path
	Op   string // operation: "read", "parse", "validate"
...
var (
	claudeGlobalRoot string  // absolute path, set via SetRoot()
...
var (
	errRootNotSet = fmt.Errorf("claude-global root not set — call SetRoot() first")
)
...
// checkRoot validates claudeGlobalRoot has been set via SetRoot().
//
// Returns:
//   - error: errRootNotSet if claudeGlobalRoot is empty, nil otherwise
//
...
func loadConfig() (*Config, error) {
	configPath := filepath.Join(claudeGlobalRoot, ConfigFile) // ConfigFile from SETUP > Constants
...
func SetRoot(path string) {
	claudeGlobalRoot = path // SETUP > Variables > Configuration State
}
...
func Load() (*Config, error) {
	...
	fmt.Println("════════════════════════════════════════════════════════════════")
	fmt.Println("⚠️  TRIPWIRE: config.toml NOT FOUND")
	fmt.Println("════════════════════════════════════════════════════════════════")
	fmt.Println("  Running on HARDCODED DEFAULTS - this is a fallback, not normal!")
	fmt.Println("  Create config.toml to use the single-source-of-truth config.")
	fmt.Println("════════════════════════════════════════════════════════════════")
...
func AgentsPath() (string, error) {
	...
	return filepath.Join(claudeGlobalRoot, anchors.Agents), nil
}
...
func HooksPath() (string, error) {
	...
	return filepath.Join(claudeGlobalRoot, anchors.Hooks), nil
}
...
func PkgPath() (string, error) {
	...
	return filepath.Join(claudeGlobalRoot, exts.Pkg), nil
}
...
func BinPath() (string, error) {
	...
	return filepath.Join(claudeGlobalRoot, exts.Bin), nil
}
...
//   - claudeGlobalRoot: Package-level state, persists until process ends
