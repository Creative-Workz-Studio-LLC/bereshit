...
// This flows: dependencies → constants → state → data model → declarations → infrastructure
//
...
//
// 3. STATIC VARIABLES (Variables)
//    Purpose: File-level mutable state (static keyword for internal linkage)
//    Subsections: State Variables → Configuration State
//
// 4. TYPES
...
//    Purpose: Forward declarations (NOT implementations - that's BODY)
//    Subsections: Helper Prototypes → Core Operation Prototypes → Public Prototypes
//
// 6. FILE-LEVEL STATE (Rails Pattern)
//    Purpose: Logging, debugging, health scoring infrastructure
//    Subsections: Rails Infrastructure → Initialization Function
...
//
//   - Includes: Rarely reserved - most files include something
//   - Defines: [Reserved: No fixed configuration values needed]
//   - Static Variables: [Reserved: Stateless - uses function parameters only]
//   - Types: [Reserved: Uses types from included headers only]
//   - Function Prototypes: [Reserved: All functions defined in order, no forward decls needed]
//   - File-Level State: [Reserved: Pure utility - no health tracking]
//
...
//   - Helpers/Utilities: [Reserved: No static helpers - uses library functions only]
//   - Core Operations: Rarely reserved - contains primary business logic
//   - Error Handling: [Reserved: Uses return codes, no custom recovery]
//   - Public APIs: [Reserved: Library-only - no extern functions in this file]
//
// Unlike METADATA (sections omitted entirely with [OMIT:]), BODY preserves
// all section headers with [Reserved:] notation for unused sections.
//
// For multi-file components:
//   - Main file: Contains Org Chart, Public APIs, Error Handling
//   - Module files: Contains static Helpers, Core Operations
//   - Document extraction with [Reserved: Extracted to module.c (main orchestrates)]
...
//    Purpose: Prove correctness before shipping - compile, test, verify
...
//    Purpose: Resource release, memory management, error state cleanup
//    Subsections: Resource Management → Memory Management → Error State Cleanup
//
// GROUP 2: FINAL DOCUMENTATION (Synthesis - Reference Back to Earlier Blocks)
//
// 4. COMPONENT OVERVIEW (Summary with Back-References)
...
//    References: SETUP constants/types, BODY function comments
//
...
//   - Code Cleanup: Manual memory management and resource release
//
