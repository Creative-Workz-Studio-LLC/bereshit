...
// ═══════════════════════════════════════════════════════════════════════════
//
// Key: cws-server-builder-handler-config
...
func (s *Service) handleReadConfig(w http.ResponseWriter, _ *http.Request) {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			server.WriteError(w, http.StatusNotFound, "Config not found")
			return
		}
	...
	if err := yaml.Unmarshal(data, &parsed); err != nil {
		server.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("YAML parse error: %s", err))
		return
	}

	server.WriteJSON(w, http.StatusOK, parsed)
}
...
func (s *Service) handleUpdateConfigSection(w http.ResponseWriter, r *http.Request) {
	...
	if section == "" {
		server.WriteError(w, http.StatusBadRequest, "section is required")
		return
	}
	...
	if err := yaml.Unmarshal(body, &sectionNode); err != nil {
		server.WriteError(w, http.StatusBadRequest, fmt.Sprintf("invalid YAML/JSON: %s", err))
		return
	}
	...
	if !updated {
		server.WriteError(w, http.StatusNotFound, fmt.Sprintf("section %q not found in config", section))
		return
	}

	output, err := yaml.Marshal(&doc)
	if err != nil {
		server.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("YAML serialize error: %s", err))
		return
	}
...
func (s *Service) handleReadPreferences(w http.ResponseWriter, _ *http.Request) {
	...
	server.WriteJSON(w, http.StatusOK, prefs)
}
...
func (s *Service) handleWritePreferences(w http.ResponseWriter, r *http.Request) {
	...
	if err := server.DecodeJSON(r, &prefs); err != nil {
	...
	server.WriteJSON(w, http.StatusOK, map[string]string{"status": "saved"})
}
